...
def get_or_create_oauth_user(db: Session, user_info: dict) -> models.User:
    """
    Finds a user by email or creates a new one for OAuth logins.
    """
    ...
    db_user = models.User(
        ...
        is_active=True         # Email is considered verified by the provider
    )
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    ...
    create_audit_log(
        db, action="user_register_oauth", user_id=db_user.id,
        details={"email": db_user.email, "username": db_user.username, "provider": "google"}
    )
    return db_user
...
def add_user_to_team(db: Session, user: models.User, team: models.Team):
    user.team_id = team.id; db.commit(); db.refresh(user); return user
def remove_user_from_team(db: Session, user: models.User):
    user.team_id = None; db.commit(); db.refresh(user); return user
...
def get_user_solved_challenge_ids(db: Session, user_id: int) -> set: return {solve.challenge_id for solve in db.query(models.Solve.challenge_id).filter(models.Solve.user_id == user_id).all()}
def get_visible_challenges(db: Session, user_id: int) -> List[models.Challenge]:
    ...
    for c in challenges: c.is_locked = not {dep.id for dep in c.dependencies}.issubset(solved_ids)
    return challenges
def get_challenge(db: Session, challenge_id: int, user_id: int) -> Optional[models.Challenge]:
    ...
    solved_ids = get_user_solved_challenge_ids(db, user_id)
    challenge.is_locked = not {dep.id for dep in challenge.dependencies}.issubset(solved_ids)
    return challenge
def create_solve(db: Session, user: models.User, challenge: models.Challenge) -> models.Solve:
    ...
    db.add(db_solve); db.commit(); db.refresh(db_solve); return db_solve
