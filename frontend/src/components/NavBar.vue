      ...
      <div class="flex justify-between items-center py-4">
        ...
        <div class="flex items-center space-x-6">
          ...
          <template v-else>
            <router-link
              to="/login"
              class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-md transition-colors"
            >
              Login
            </router-link>
            <router-link
...
const handleLogout = () => {
  authStore.logout();
  // The store's logout action already redirects to the login page.
};
</script>
